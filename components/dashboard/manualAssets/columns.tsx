...
export function getManualAssetTableColumns(
  dashboardAmountVisibility: boolean,
  conversionRates: TConversionRates
) {
  const manualAssetTableColumns: ColumnDef<TAsset>[] = [
    {
      accessorKey: "name",
      header: "Name",
    },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const currPrice = +row.original.buyPrice;
        const buyCurrency = row.original.buyCurrency.toLowerCase();
        const conversionRate = conversionRates[buyCurrency];

        const currentPrice = (currPrice / conversionRate).toFixed(2);
        return <div className="text-right">{currentPrice}</div>;
      },
      sortingFn: (rowA, rowB) =>
        +rowA.original.currentPrice - +rowB.original.currentPrice,
    },
    {
  ...
  return manualAssetTableColumns;
}
